...
def eval(input_vector,binary_v):
...
def search_SA(input_v,sol):  # sol is a boolean array and 0 represents absence of input feature 1 repesent presence of the same
    ...
    alpha=0.75
    while (T > T_min):
        e = random.randint(0,n-1)   #finding a random neighbour
        if sol[0][e]==True:
            next_sol[0][e]=False
        else:
            next_sol[0][e]=True 
        
        #print("actual:",next_sol,sol)
        #print("Not: ",not any(next_sol[0]),not any(sol[0])) 
        e1=eval(input_v,sol)
        e2=eval(input_v,next_sol)
        delta_E =  e2- e1
        y = random.random()
        if delta_E>0:
            sol=copy.copy(next_sol)
        elif probabilty(delta_E,T)>y:
            sol=copy.copy(next_sol)  # moving to next_sol with probabilty prob(delta_E,T)
        T=alpha*T
        acc=eval(input_v,sol)
        y_value.append(acc)
